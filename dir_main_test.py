...
class CritiGraph(torch.nn.Module):
    ...
    def loom(self, epoch, sta_ind):
        ...
        with torch.no_grad():
            ...
            for j in range(2):
                pos_loc[j][mask[j]] = self.locations[j][pos_ind[j][mask[j]]]
            ...
            for j in range(2):
                neg_loc[j][mask[j]] = self.locations[j][neg_ind[j][mask[j]]]
            ...
            mask2 = [mask1[j].unsqueeze(2).repeat(1, 1, cnc_loc[j//2].size(1),1) for j in range(2)]
    
            dis_sta_pos = [torch.where(mask1[0], self.distance(sta_loc[0][:,None,:], pos_loc[0]), -1),
                           torch.where(mask1[0], self.distance(sta_loc[0][:,None,:], pos_loc[1]), -1),
                           torch.where(mask1[0], self.distance(sta_loc[1][:,None,:], pos_loc[0]), -1),
                           torch.where(mask1[0], self.distance(sta_loc[1][:,None,:], pos_loc[1]), -1),
                           torch.where(mask1[1], self.distance(sta_loc[0][:,None,:], pos_loc[0]), -1),
                            torch.where(mask1[1], self.distance(sta_loc[1][:,None,:], pos_loc[0]), -1)]
            dis_sta_posum = [torch.sum(dis_sta_pos[j], dim=-1) for j in range(4)]
            dis_sta_neg = [torch.where(mask1[0], self.distance(sta_loc[0][:,None,:], neg_loc[0]), -1),
                           torch.where(mask1[0], self.distance(sta_loc[0][:,None,:], neg_loc[1]), -1),
                           torch.where(mask1[1], self.distance(sta_loc[0][:,None,:], neg_loc[0]), -1),
                            torch.where(mask1[1], self.distance(sta_loc[1][:,None,:], neg_loc[0]), -1)]
            dis_sta_negum = [torch.sum(dis_sta_neg[j], dim=-1) for j in range(4)]
            dis_pos_cnc = [torch.where(mask2[0], self.distance(cnc_loc[0][:,None,:,:], pos_loc[0][:,:,None,:]), -1),
                           torch.where(mask2[0], self.distance(cnc_loc[0][:,None,:,:], pos_loc[1][:,:,None,:]), -1),
                           torch.where(mask2[1], self.distance(cnc_loc[0][:,None,:,:], pos_loc[0][:,:,None,:]), -1),
                            torch.where(mask2[1], self.distance(cnc_loc[1][:,None,:,:], pos_loc[0][:,:,None,:]), -1)]
            dis_neg_cnc = [torch.where(mask2[0], self.distance(cnc_loc[0][:,None,:,:], neg_loc[0][:,:,None,:]), -1),
                           torch.where(mask2[0], self.distance(cnc_loc[0][:,None,:,:], neg_loc[1][:,:,None,:]), -1),
                           torch.where(mask2[1], self.distance(cnc_loc[0][:,None,:,:], neg_loc[0][:,:,None,:]), -1),
                            torch.where(mask2[1], self.distance(cnc_loc[1][:,None,:,:], neg_loc[0][:,:,None,:]), -1)]
            dis_new_pos_4 = [torch.where(mask2[j//2], (dis_pos_cnc[j]-dis_sta_pos[j][:,:,None,:]+dis_sta_posum[j][:,:,None,None])/self.tp, 0) for j in range(4)]
            dis_new_pos = [dis_new_pos_4[0]*ratio+dis_new_pos_4[1]*(1-ratio),
                           dis_new_pos_4[2]*ratio+dis_new_pos_4[3]*(1-ratio)]
            ...
            dis_new_neg_4 = [torch.where(mask2[j//2], (dis_neg_cnc[j]-dis_sta_neg[j][:,:,None,:]+dis_sta_negum[j][:,:,None,None])/self.tp, 1000) for j in range(4)]
            dis_new_neg = [dis_new_neg_4[0]*ratio+dis_new_neg_4[1]*(1-ratio),
                           dis_new_neg_4[2]*ratio+dis_new_neg_4[3]*(1-ratio)]
            neg_loss = [-torch.sum(torch.where(mask2[j], torch.log(self.eps+self.P(dis_new_neg[j], sta_ind[:,None], neg_ind[j], 'neg')), torch.tensor(0.0, device=device)), dim=1) / lg[j][:,None,None] for j in range(2)]
    ...
    def get_score(self, data):
        ...
        for i in range(self.chunks):
            ...
            chunk_vector2 = self.locations[0][chunk_dt2]
            chunk_vector22 = self.locations[1][chunk_dt2]
            dis1 = self.distance(chunk_vector1, chunk_vector2).float()
            dis2 = self.distance(chunk_vector1, chunk_vector22).float()
            dis = ratio * dis1 + (1 - ratio) * dis2
