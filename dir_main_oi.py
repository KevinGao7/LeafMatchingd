...
class CritiGraph(torch.nn.Module):
    ...
    def get_neighbor(self):
        neighbor_all = [torch.tensor(list(set(self.G.predecessors(ii)) | set(self.G.successors(ii))), dtype=torch.int64, device=device) 
        ...
        neighbor_tensor = [torch.full((self.num_nodes, self.max_degree), -1, dtype=torch.int64, device=device) for _ in range(3)]
        for j in range(3):
