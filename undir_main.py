...
from utils import eval_hits, eval_auc_ap_f1
import pandas as pd
...
if __name__ == "__main__":
    ...
    print(f">>> saving embedding to {save_emb} >>>")
    ...
    # 8. test performance
    roc, pr, f1 = eval_auc_ap_f1(torch.sort(pos)[0], torch.sort(neg)[0])
    hits20 = eval_hits(torch.sort(pos)[0], torch.sort(neg)[0], K=20)
    hits50 = eval_hits(torch.sort(pos)[0], torch.sort(neg)[0], K=50)
    hits100 = eval_hits(torch.sort(pos)[0], torch.sort(neg)[0], K=100)
    
    print("current_time:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    print(f">>> finished testing >>>")
    
    # 9. save results
    test_results = [[split_ratio, roc, pr, f1, hits20, hits50, hits100]]
    test_results_df = pd.DataFrame(test_results, columns=['split_ratio', 'roc_auc', 'pr_auc', 'f1',
                                                          'hits20', 'hits50', 'hits100'])
    test_results_df.to_csv('./results/' + f'{dataset_name}-r{split_ratio}-t{turn}-speedup.csv', index=False)
