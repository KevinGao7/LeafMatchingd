...
class CritiGraph(torch.nn.Module):
    ...
    def loom(self, epoch, sta_ind):
        ...
        with torch.no_grad():
            ...
            # print(0, mask1[0].shape, mask2[0].shape)
            ...
            # print(1, dis_pos_cnc[0].shape, dis_neg_cnc[0].shape)
            dis_new_pos = [torch.where(mask2[j], (dis_pos_cnc[j]-dis_sta_pos[j][:,:,None,:]+dis_sta_posum[j][:,:,None,None])/self.tp, 0) for j in range(4)]
            dis_new_neg = [torch.where(mask2[j], (dis_neg_cnc[j]-dis_sta_neg[j][:,:,None,:]+dis_sta_negum[j][:,:,None,None])/self.tp, 1000) for j in range(4)]
            
            p_pos = [torch.where(mask2[j], self.P(dis_new_pos[j], sta_ind[:,None], pos_ind[j//2], j//2), torch.tensor(0.0, device=device)) for j in range(4)]
            p_neg = [torch.where(mask2[j], self.P(dis_new_neg[j], sta_ind[:,None], neg_ind[j//2], j//2+2), torch.tensor(0.0, device=device)) for j in range(4)]
            pos_loss = [-torch.sum(torch.where(mask2[0], torch.log(self.eps+ratio*p_pos[0]+(1-ratio)*p_pos[1]), torch.tensor(0.0, device=device)), dim=1) / lg[0][:,None,None]\
                         -torch.sum(torch.where(mask2[2], torch.log(self.eps+p_pos[2]), torch.tensor(0.0, device=device)), dim=1) / lg[1][:,None,None],
                         -torch.sum(torch.where(mask2[3], torch.log(self.eps+p_pos[3]), torch.tensor(0.0, device=device)), dim=1) / lg[1][:,None,None]]
            neg_loss = [-torch.sum(torch.where(mask2[0], torch.log(self.eps+ratio*p_neg[0]+(1-ratio)*p_neg[1]), torch.tensor(0.0, device=device)), dim=1) / lg[0][:,None,None]\
                         -torch.sum(torch.where(mask2[2], torch.log(self.eps+p_neg[2]), torch.tensor(0.0, device=device)), dim=1) / lg[1][:,None,None],
    ...
    def get_score(self, data):
        ...
        for i in range(self.chunks):
            ...
            dis_1 = self.distance(chunk_vector1, chunk_vector2).float()
            dis_2 = self.distance(chunk_vector1, chunk_vector22).float()
            dis_1_mean = torch.mean(dis_1, dim=-1)
            dis_2_mean = torch.mean(dis_2, dim=-1)
            pp_chunk_1 = self.p_test(dis_1_mean, chunk_dt1, chunk_dt2)
            pp_chunk_2 = self.p_test(dis_2_mean, chunk_dt1, chunk_dt2)
            pp_chunk = ratio * pp_chunk_1 + (1 - ratio) * pp_chunk_2
