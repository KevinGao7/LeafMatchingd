...
for gamma in 6
do
    for alpha_i in 1 2 3 4 5 6
    do
        if [ $alpha_i -eq 1 ]; then
            alphas=(0.1 0.25 0.5 0.8)
        elif [ $alpha_i -eq 2 ]; then
            alphas=(1 3 8 15)
        elif [ $alpha_i -eq 3 ]; then
            alphas=(27 40 60 80)
        elif [ $alpha_i -eq 4 ]; then
            alphas=(95 110 125 140)
        elif [ $alpha_i -eq 5 ]; then
            alphas=(160 180 220 260)
        elif [ $alpha_i -eq 6 ]; then
            alphas=(300 350 400 450)
        fi

        echo "Processing gamma: $gamma, alphas: ${alphas[@]}"
        ...
        do
            ...
            > ./grid_${dataset}/logs_${id}/s${seed}-h${h}-c${c}-p${pos_ratio}-r${ratio}-g${gamma}-a${alpha}-e${epoch}-con${convergence}.log 2>&1 &
