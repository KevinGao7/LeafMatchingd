...
class CritiGraph(torch.nn.Module):
    ...
    def loom(self, epoch, sta_ind):
        ...
        sta_loc = [self.locations[self.cs(j)][sta_ind] for j in range(6)]
        with torch.no_grad():
            pos_loc = [torch.full((pos_ind[j//3].size(0), pos_ind[j//3].size(1), self.tp), -1, dtype=torch.int64, device=device) for j in range(6)]
            for j in range(6):
                pos_loc[j][mask[j//3]] = self.locations[self.ce(j)][pos_ind[j//3][mask[j//3]]]
            ...
            neg_loc = [torch.full((pos_ind[j//3].size(0), pos_ind[j//3].size(1), self.tp), -1, dtype=torch.int64, device=device) for j in range(6)]
            for j in range(6):
                neg_loc[j][mask[j//3]] = self.locations[self.ce(j)][neg_ind[j//3][mask[j//3]]]
            ...
            cnc_loc = [cnc_loc_2[self.cs(j)][:, indices[self.cs(j)], :] for j in range(6)]
            
            mask1 = [mask[j//3].unsqueeze(2).repeat(1, 1, self.tp) for j in range(6)]
            mask2 = [mask1[j].unsqueeze(2).repeat(1, 1, cnc_loc[j].size(1),1) for j in range(6)]
            dis_sta_pos = [torch.where(mask1[j], self.distance(sta_loc[j][:,None,:], pos_loc[j]), -1) for j in range(6)]
            dis_sta_posum = [torch.sum(dis_sta_pos[j], dim=-1) for j in range(6)]
            dis_sta_neg = [torch.where(mask1[j], self.distance(sta_loc[j][:,None,:], neg_loc[j]), -1) for j in range(6)]
            dis_sta_negum = [torch.sum(dis_sta_neg[j], dim=-1) for j in range(6)]
            dis_pos_cnc = [torch.where(mask2[j], self.distance(cnc_loc[j][:,None,:,:], pos_loc[j][:,:,None,:]), -1) for j in range(6)]
            dis_neg_cnc = [torch.where(mask2[j], self.distance(cnc_loc[j][:,None,:,:], neg_loc[j][:,:,None,:]), -1) for j in range(6)]
            dis_new_pos_6 = [torch.where(mask2[j], (dis_pos_cnc[j]-dis_sta_pos[j][:,:,None,:]+dis_sta_posum[j][:,:,None,None])/self.tp, 0) for j in range(6)]
            dis_new_neg_6 = [torch.where(mask2[j], (dis_neg_cnc[j]-dis_sta_neg[j][:,:,None,:]+dis_sta_negum[j][:,:,None,None])/self.tp, 1000) for j in range(6)]
#head-tail张量升维
            dis_new_pos = [ratio*(dis_new_pos_6[0]+dis_sta_pos[1][:,:,None,:])/2+(1-ratio)*dis_new_pos_6[2], 
                           ratio*(dis_sta_pos[0][:,:,None,:]+dis_new_pos_6[1])/2+(1-ratio)*dis_sta_pos[2][:,:,None,:],
                           ratio*(dis_new_pos_6[3]+dis_sta_pos[4][:,:,None,:])/2+(1-ratio)*dis_sta_pos[5][:,:,None,:],
                           ratio*(dis_sta_pos[3][:,:,None,:]+dis_new_pos_6[4])/2+(1-ratio)*dis_new_pos_6[5]]
            dis_new_neg = [ratio*(dis_new_neg_6[0]+dis_sta_neg[1][:,:,None,:])/2+(1-ratio)*dis_new_neg_6[2],
                           ratio*(dis_sta_neg[0][:,:,None,:]+dis_new_neg_6[1])/2+(1-ratio)*dis_sta_neg[2][:,:,None,:],
                           ratio*(dis_new_neg_6[3]+dis_sta_neg[4][:,:,None,:])/2+(1-ratio)*dis_sta_neg[5][:,:,None,:],
                           ratio*(dis_sta_neg[3][:,:,None,:]+dis_new_neg_6[4])/2+(1-ratio)*dis_new_neg_6[5]]
            #此处之后要更改
            
            pos_loss_4 = [-torch.sum(torch.where(mask2[j+1], torch.log(self.eps+self.P(dis_new_pos[j], sta_ind[:,None], pos_ind[j//2], j//2)), torch.tensor(0.0, device=device)), dim=1) / lg[j//2][:,None,None] for j in range(4)]
            neg_loss_4 = [-torch.sum(torch.where(mask2[j+1], torch.log(self.eps+self.P(dis_new_neg[j], sta_ind[:,None], neg_ind[j//2], j//2+2)), torch.tensor(0.0, device=device)), dim=1) / lg[j//2][:,None,None] for j in range(4)]
            pos_loss = [pos_loss_4[0]+pos_loss_4[2], pos_loss_4[1]+pos_loss_4[3]]
            neg_loss = [neg_loss_4[0]+neg_loss_4[2], neg_loss_4[1]+neg_loss_4[3]]
            ...
            self.locations[1][sta_ind[i_indices_tail], j_indices_tail] = cnc_loc[5][i_indices_tail, index[1][i_indices_tail, j_indices_tail], j_indices_tail]
    ...
    def get_score(self, data):
        ...
        for i in range(self.chunks):
            ...
            dis = ratio * (dis_hh + dis_tt) / 2 + (1 - ratio) * dis_ht
