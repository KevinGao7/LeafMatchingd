...
class CritiGraph(torch.nn.Module):
    ...
    def loom(self, epoch, sta_ind):
        ...
        with torch.no_grad():
            ...
            pos_loss_4 = [-torch.sum(torch.where(mask2[j], torch.log(self.eps+p_pos[j]), torch.tensor(0.0, device=device)), dim=1) / lg[j//2][:,None,None] for j in range(4)]
            neg_loss_4 = [-torch.sum(torch.where(mask2[j], torch.log(self.eps+p_neg[j]), torch.tensor(0.0, device=device)), dim=1) / lg[j//2][:,None,None] for j in range(4)]
            
            pos_loss = [ratio*pos_loss_4[0]+(1-ratio)*pos_loss_4[1]+ratio*pos_loss_4[2],(1-ratio)*pos_loss_4[3]]
            neg_loss = [ratio*neg_loss_4[0]+(1-ratio)*neg_loss_4[1]+ratio*neg_loss_4[2],(1-ratio)*neg_loss_4[3]]  
    ...
    def get_score(self, data):
        ...
        for i in range(self.chunks):
            ...
            pp_chunk = ratio * torch.log(self.eps+pp_chunk_1) + (1 - ratio) * torch.log(self.eps+pp_chunk_2)
