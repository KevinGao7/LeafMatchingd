...
import pandas as pd
import torch
from OurCogDL.cogdl.wrappers.data_wrapper.link_prediction.embedding_link_prediction_dw_dir2 import \
    EmbeddingLinkPredictionDataWrapper
from OurCogDL.cogdl.wrappers.data_wrapper.link_prediction.embedding_link_prediction_dw_cit2 import \
...
def load_icews18(root, type: Literal['min', 'mid', 'max']='min'):
    # 加载训练、验证和测试数据
...
def transform_fuckyou(train_data, test_data, node_feat=None):
    """红温了"""
    return train_data, test_data[0], test_data[1]
...
def split_dataset(name, split_ratio, data_type):
    ...
    if name == 'cora_ml':
        ...
        split_subdataset_path = osp.join("./datasets", "icews18", 'split_' + name.split('_')[-1])
    elif name == 'amazon':
    ...
    # >>> Transform and Save >>> #
    train_pos, test_pos, test_neg = transform_fuckyou(train_data, test_data, node_feat=node_feat)
    logging.info(f"Train Edges: {train_pos.shape[1]}, Test Pos Edges: {test_pos.shape[1]}, Test Neg Edges: {test_neg.shape[1]}")
    logging.info(f"Train/Test Pos Edges: {train_pos.shape[1] / (test_pos.shape[1])}")
    logging.info(f"Train Edges/Nodes: {train_pos.shape[1]} / {len(torch.unique(train_data))} = {train_pos.shape[1] / len(torch.unique(train_data))}\n")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename=f'./datasets/logs/_{dataset}_splitting.log', level=logging.INFO, filemode='a')
