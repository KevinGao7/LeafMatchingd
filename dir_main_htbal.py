...
class CritiGraph(torch.nn.Module):
    ...
    def loom(self, epoch, sta_ind):
        ...
        with torch.no_grad():
            ...
            dis_new_pos = [torch.where(mask2[j], (dis_pos_cnc[j]-dis_sta_pos[j][:,:,None,:]+dis_sta_posum[j][:,:,None,None])/self.tp, 0) for j in range(6)]
            dis_new_neg = [torch.where(mask2[j], (dis_neg_cnc[j]-dis_sta_neg[j][:,:,None,:]+dis_sta_negum[j][:,:,None,None])/self.tp, 1000) for j in range(6)]
            
            p_pos = [torch.where(mask2[j], self.P(dis_new_pos[j], sta_ind[:,None], pos_ind[j//3], j//3), torch.tensor(0.0, device=device)) for j in range(6)]
            p_neg = [torch.where(mask2[j], self.P(dis_new_neg[j], sta_ind[:,None], neg_ind[j//3], j//3+2), torch.tensor(0.0, device=device)) for j in range(6)]
            pos_loss_6 = [-torch.sum(torch.where(mask2[j], torch.log(self.eps+p_pos[j]), torch.tensor(0.0, device=device)), dim=1) / lg[j//3][:,None,None] for j in range(6)]
            neg_loss_6 = [-torch.sum(torch.where(mask2[j], torch.log(self.eps+p_neg[j]), torch.tensor(0.0, device=device)), dim=1) / lg[j//3][:,None,None] for j in range(6)]
            
            pos_loss = [ratio*(pos_loss_6[0]+pos_loss_6[3])/2+(1-ratio)*pos_loss_6[1], ratio*(pos_loss_6[2]+pos_loss_6[5])/2+(1-ratio)*pos_loss_6[4]]
            neg_loss = [ratio*(neg_loss_6[0]+neg_loss_6[3])/2+(1-ratio)*neg_loss_6[1], ratio*(neg_loss_6[2]+neg_loss_6[5])/2+(1-ratio)*neg_loss_6[4]]  
           
            total_loss = [self.pos_ratio * pos_loss[j] + neg_loss[j] for j in range(2)]
    ...
    def get_score(self, data):
        ...
        for i in range(self.chunks):
            ...
            pp_chunk = ratio * (torch.log(self.eps+pp_chunk_1) + torch.log(self.eps+pp_chunk_3)) / 2 + (1 - ratio) * torch.log(self.eps+pp_chunk_2)
